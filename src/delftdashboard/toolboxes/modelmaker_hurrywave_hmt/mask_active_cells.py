"""GUI callbacks for the HurryWave Model Maker Mask Active Cells tab.

Handles include/exclude polygon drawing, loading, saving, and
triggering mask updates and inactive cell removal.
"""
...
def select(*args: Any) -> None:
    """Activate the Mask Active Cells tab and show relevant layers."""
    map.update()
...
def deselect(*args: Any) -> None:
    """Prompt to save unsaved polygon changes on tab switch."""
    changed = False
    if (
...
def draw_include_polygon(*args: Any) -> None:
    """Start interactive drawing of an include polygon."""
...
def delete_include_polygon(*args: Any) -> None:
    ...
    if index > len(app.toolbox[_TB].include_polygon) - 1:
        app.gui.setvar(
            _TB, "include_polygon_index", len(app.toolbox[_TB].include_polygon) - 1
        )
    update()


def load_include_polygon(*args: Any) -> None:
    """Load include polygons from a GeoJSON file."""
...
def select_include_polygon(*args: Any) -> None:
    """Highlight the selected include polygon on the map."""
    index = args[0]
...
def include_polygon_created(gdf: gpd.GeoDataFrame, index: int, id: Any) -> None:
    ...
    gdf : gpd.GeoDataFrame
        Updated GeoDataFrame with all include polygons.
    index : int
...
def include_polygon_modified(gdf: gpd.GeoDataFrame, index: int, id: Any) -> None:
    """Handle modification of an include polygon.
...
def include_polygon_selected(index: int) -> None:
    """Sync the GUI when an include polygon is selected on the map.
...
def draw_exclude_polygon(*args: Any) -> None:
    """Start interactive drawing of an exclude polygon."""
...
def delete_exclude_polygon(*args: Any) -> None:
    ...
    if index > len(app.toolbox[_TB].exclude_polygon) - 1:
        app.gui.setvar(
            _TB, "exclude_polygon_index", len(app.toolbox[_TB].exclude_polygon) - 1
        )
    update()


def load_exclude_polygon(*args: Any) -> None:
    """Load exclude polygons from a GeoJSON file."""
    full_name, path, name, ext, fltr = app.gui.window.dialog_open_file(
...
def select_exclude_polygon(*args: Any) -> None:
    """Highlight the selected exclude polygon on the map."""
    index = args[0]
...
def exclude_polygon_created(gdf: gpd.GeoDataFrame, index: int, id: Any) -> None:
    ...
    ----------
    gdf : gpd.GeoDataFrame
        Updated GeoDataFrame with all exclude polygons.
    index : int
        Index of the created feature.
    id : Any
        Feature identifier.
...
def exclude_polygon_modified(gdf: gpd.GeoDataFrame, index: int, id: Any) -> None:
    """Handle modification of an exclude polygon.
...
def exclude_polygon_selected(index: int) -> None:
    """Sync the GUI when an exclude polygon is selected on the map.
...
def update() -> None:
    """Refresh include/exclude polygon counts and names in the GUI."""
    nrp = len(app.toolbox[_TB].include_polygon)
