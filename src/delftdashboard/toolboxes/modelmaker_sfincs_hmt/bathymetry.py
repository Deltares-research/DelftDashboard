...
def use_dataset(*args):
    ...
    if name not in app.gui.getvar(group, "selected_bathymetry_dataset_names"):
        ...
        app.gui.setvar(group, "selected_bathymetry_dataset_index", len(app.toolbox["modelmaker_sfincs_hmt"].selected_bathymetry_datasets) - 1)
        update()
...
def move_down_selected_bathymetry_dataset(*args):
    ...
    group = "modelmaker_sfincs_hmt"
    index = app.gui.getvar(group, "selected_bathymetry_dataset_index")
    if index == len(app.toolbox["modelmaker_sfincs_hmt"].selected_bathymetry_datasets) - 1:
        return
    i0 = index
    i1 = index + 1
    app.toolbox["modelmaker_sfincs_hmt"].selected_bathymetry_datasets[i0],\
...
def edit_zmax_bathymetry_dataset(*args):
    group = "modelmaker_sfincs_hmt"
    index = app.gui.getvar(group, "selected_bathymetry_dataset_index")
    app.toolbox["modelmaker_sfincs_hmt"].selected_bathymetry_datasets[index]["zmax"] = args[0]
...
def update():
    ...
    if nrd>0:
        ...
        if index > nrd - 1:
            index = nrd - 1
        dataset = app.toolbox["modelmaker_sfincs_hmt"].selected_bathymetry_datasets[index]
