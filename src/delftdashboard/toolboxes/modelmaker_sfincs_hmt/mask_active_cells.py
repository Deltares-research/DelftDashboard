...
def select(*args):
    ...
    app.map.layer["modelmaker_sfincs_hmt"].layer["include_polygon"].activate()
    app.map.layer["modelmaker_sfincs_hmt"].layer["exclude_polygon"].activate()
...
def deselect(*args):
    changed = False
    if app.toolbox["modelmaker_sfincs_hmt"].include_polygon_changed and len(app.toolbox["modelmaker_sfincs_hmt"].include_polygon) > 0:
        changed = True
    if app.toolbox["modelmaker_sfincs_hmt"].exclude_polygon_changed and len(app.toolbox["modelmaker_sfincs_hmt"].exclude_polygon) > 0:
        changed = True
    if changed:    
...
def draw_include_polygon(*args):
    app.map.layer["modelmaker_sfincs_hmt"].layer["include_polygon"].crs = app.crs
    app.map.layer["modelmaker_sfincs_hmt"].layer["include_polygon"].draw()

def delete_include_polygon(*args):
    if len(app.toolbox["modelmaker_sfincs_hmt"].include_polygon) == 0:
        return
    index = app.gui.getvar("modelmaker_sfincs_hmt", "include_polygon_index")
    gdf = app.map.layer["modelmaker_sfincs_hmt"].layer["include_polygon"].delete_feature(index)
    app.toolbox["modelmaker_sfincs_hmt"].include_polygon = gdf
    app.toolbox["modelmaker_sfincs_hmt"].include_polygon_changed = True
    update()
...
def load_include_polygon(*args):
    ...
    if not full_name:
        return
    append = False
    if app.gui.getvar("modelmaker_sfincs_hmt", "nr_include_polygons"):
        append = app.gui.window.dialog_yes_no("Add to existing include polygons?", " ")
    app.toolbox["modelmaker_sfincs_hmt"].read_include_polygon(full_name, append)
    app.toolbox["modelmaker_sfincs_hmt"].plot_include_polygon()
    if append:
        app.toolbox["modelmaker_sfincs_hmt"].include_polygon_changed = True
    else:
        save_include_polygon()    
...
def save_include_polygon(*args):
    ...
    app.toolbox["modelmaker_sfincs_hmt"].include_polygon_changed = False

def select_include_polygon(*args):
    index = args[0]
    app.map.layer["modelmaker_sfincs_hmt"].layer["include_polygon"].activate_feature(index)

def include_polygon_created(gdf, index, id):
    app.toolbox["modelmaker_sfincs_hmt"].include_polygon = gdf
    nrp = len(app.toolbox["modelmaker_sfincs_hmt"].include_polygon)
    app.gui.setvar("modelmaker_sfincs_hmt", "include_polygon_index", nrp - 1)
    app.toolbox["modelmaker_sfincs_hmt"].include_polygon_changed = True
    update()
...
def include_polygon_selected(index):
    # Selected from map
    app.gui.setvar("modelmaker_sfincs_hmt", "include_polygon_index", index)
    update()

def draw_exclude_polygon(*args):
    app.map.layer["modelmaker_sfincs_hmt"].layer["exclude_polygon"].draw()

def delete_exclude_polygon(*args):
    if len(app.toolbox["modelmaker_sfincs_hmt"].exclude_polygon) == 0:
        return
    index = app.gui.getvar("modelmaker_sfincs_hmt", "exclude_polygon_index")
    # Delete from map
    gdf = app.map.layer["modelmaker_sfincs_hmt"].layer["exclude_polygon"].delete_feature(index)
    app.toolbox["modelmaker_sfincs_hmt"].exclude_polygon = gdf
    app.toolbox["modelmaker_sfincs_hmt"].exclude_polygon_changed = True
    update()
...
def load_exclude_polygon(*args):
    ...
    if not full_name:
        return
    append = False
    if app.gui.getvar("modelmaker_sfincs_hmt", "nr_exclude_polygons"):
        append = app.gui.window.dialog_yes_no("Add to existing exclude polygons?", " ")
    app.toolbox["modelmaker_sfincs_hmt"].read_exclude_polygon(full_name, append)
    if append:
        app.toolbox["modelmaker_sfincs_hmt"].exclude_polygon_changed = True
    else:
        save_exclude_polygon()
    app.toolbox["modelmaker_sfincs_hmt"].plot_exclude_polygon()
    update()
...
def save_exclude_polygon(*args):
    ...
    app.toolbox["modelmaker_sfincs_hmt"].exclude_polygon_changed = False

def select_exclude_polygon(*args):
    index = args[0]
    app.map.layer["modelmaker_sfincs_hmt"].layer["exclude_polygon"].activate_feature(index)

def exclude_polygon_created(gdf, index, id):
    app.toolbox["modelmaker_sfincs_hmt"].exclude_polygon = gdf
    nrp = len(app.toolbox["modelmaker_sfincs_hmt"].exclude_polygon)
    app.gui.setvar("modelmaker_sfincs_hmt", "exclude_polygon_index", nrp - 1)
    app.toolbox["modelmaker_sfincs_hmt"].exclude_polygon_changed = True
    update()
...
def exclude_polygon_selected(index):
    app.gui.setvar("modelmaker_sfincs_hmt", "exclude_polygon_index", index)
    update()
