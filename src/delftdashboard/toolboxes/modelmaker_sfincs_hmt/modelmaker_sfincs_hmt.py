...
class Toolbox(GenericToolbox):
    ...
    def generate_grid(self):
        ...
        app.gui.setvar("sfincs_hmt", "qtrfile", model.input.variables.qtrfile)
    ...
    def generate_bathymetry(self):
        ...
        bathymetry_list = app.toolbox["modelmaker_sfincs_hmt"].selected_bathymetry_datasets
        app.model["sfincs_hmt"].domain.grid.set_bathymetry(bathymetry_list,
                                                           bathymetry_database=app.bathymetry_database,
        ...
        app.model["sfincs_hmt"].domain.grid.write()
        # # If SnapWave also generate SnapWave mesh and save it
    ...
    def update_mask(self):
        ...
        z    = app.model["sfincs_hmt"].domain.grid.data["z"]
        if np.all(np.isnan(z)):
            app.gui.window.dialog_warning("Please first generate a bathymetry !")
            return
        dlg = app.gui.window.dialog_wait("Updating mask ...")
        if app.gui.getvar("modelmaker_sfincs_hmt", "use_mask_global"):
            global_zmin = app.gui.getvar("modelmaker_sfincs_hmt", "global_zmin")
            global_zmax = app.gui.getvar("modelmaker_sfincs_hmt", "global_zmax")
        else:
    ...
    def update_mask_snapwave(self):
        grid = app.model["sfincs_hmt"].domain.grid
        mask = app.model["sfincs_hmt"].domain.snapwave.mask
    ...
    def generate_subgrid(self):
        ...
        app.model["sfincs_hmt"].domain.subgrid.build(bathymetry_sets,
                                                     roughness_sets,
                                                     bathymetry_database=app.bathymetry_database,
                                                     ...
                                                     progress_bar=p)
        p.close()
        app.model["sfincs_hmt"].domain.input.variables.sbgfile = "sfincs.sbg"
        app.gui.setvar("sfincs_hmt", "sbgfile", app.model["sfincs_hmt"].domain.input.variables.sbgfile)
    ...
    def cut_inactive_cells(self):
        ...
        app.model["sfincs_hmt"].domain.grid.cut_inactive_cells()
        app.model["sfincs_hmt"].domain.grid.write()
        # Replot everything
        app.model["sfincs_hmt"].plot()
        dlg.close()
    ...
    def build_model(self):
        ...
        if app.gui.getvar("modelmaker_sfincs_hmt", "use_subgrid"):
    ...
    def show_mask_polygons(self):
        ...
        if show_polygons:
            ...
            app.map.layer["modelmaker_sfincs_hmt"].layer["include_polygon"].deactivate()
            app.map.layer["modelmaker_sfincs_hmt"].layer["exclude_polygon"].deactivate()
        else:
            app.map.layer["modelmaker_sfincs_hmt"].layer["include_polygon"].hide()
            app.map.layer["modelmaker_sfincs_hmt"].layer["exclude_polygon"].hide()
    ...
    def plot_include_polygon(self):
        layer = app.map.layer["modelmaker_sfincs_hmt"].layer["include_polygon"]
        layer.clear()
        layer.add_feature(self.include_polygon)

    def plot_exclude_polygon(self):
        layer = app.map.layer["modelmaker_sfincs_hmt"].layer["exclude_polygon"]
        layer.clear()
        layer.add_feature(self.exclude_polygon)

    def plot_open_boundary_polygon(self):
        layer = app.map.layer["modelmaker_sfincs_hmt"].layer["open_boundary_polygon"]
        layer.clear()
    ...
    def plot_include_polygon_snapwave(self):
        layer = app.map.layer["modelmaker_sfincs_hmt"].layer["include_polygon_snapwave"]
        layer.clear()
        layer.add_feature(self.include_polygon_snapwave)

    def plot_exclude_polygon_snapwave(self):
        layer = app.map.layer["modelmaker_sfincs_hmt"].layer["exclude_polygon_snapwave"]
        layer.clear()
    ...
    def read_setup_yaml(self, file_name):
        ...
        app.model["sfincs_hmt"].domain.crs = CRS(dct["coordinates"]["crs"])

        # Change coordinate system
        map.set_crs(app.model["sfincs_hmt"].domain.crs)
        ...
        # Sub grid
        if "subgrid" in dct:
            app.gui.setvar("modelmaker_sfincs_hmt", "use_subgrid", True)
        else:
