...
def remove_selected_bathymetry_dataset(*args):
    if len(app.toolbox["modelmaker_delft3dfm"].selected_bathymetry_datasets) == 0:
        return
    group = "modelmaker_delft3dfm"
    index = app.gui.getvar(group, "selected_bathymetry_dataset_index")
    app.toolbox["modelmaker_delft3dfm"].selected_bathymetry_datasets.pop(index)
    update()


def  move_up_selected_bathymetry_dataset(*args):
    if len(app.toolbox["modelmaker_delft3dfm"].selected_bathymetry_datasets) < 2:
        return
    group = "modelmaker_delft3dfm"
    index = app.gui.getvar(group, "selected_bathymetry_dataset_index")
    if index == 0:
    ...
    i0 = index
    i1 = index - 1
    app.toolbox["modelmaker_delft3dfm"].selected_bathymetry_datasets[i0],\
    app.toolbox["modelmaker_delft3dfm"].selected_bathymetry_datasets[i1] = \
    app.toolbox["modelmaker_delft3dfm"].selected_bathymetry_datasets[i1], \
    app.toolbox["modelmaker_delft3dfm"].selected_bathymetry_datasets[i0]
...
def move_down_selected_bathymetry_dataset(*args):
    ...
    group = "modelmaker_delft3dfm"
    index = app.gui.getvar(group, "selected_bathymetry_dataset_index")
    if index == len(app.toolbox["modelmaker_delft3dfm"].selected_bathymetry_datasets) - 1:
        return
    i0 = index
    i1 = index + 1
    app.toolbox["modelmaker_delft3dfm"].selected_bathymetry_datasets[i0],\
...
def edit_zmax_bathymetry_dataset(*args):
    group = "modelmaker_delft3dfm"
    index = app.gui.getvar(group, "selected_bathymetry_dataset_index")
    app.toolbox["modelmaker_delft3dfm"].selected_bathymetry_datasets[index]["zmax"] = args[0]


def edit_zmin_bathymetry_dataset(*args):
    group = "modelmaker_delft3dfm"
    index = app.gui.getvar(group, "selected_bathymetry_dataset_index")
    app.toolbox["modelmaker_delft3dfm"].selected_bathymetry_datasets[index]["zmin"] = args[0]


def update():
    group = "modelmaker_delft3dfm"
    selected_names = []
    nrd = len(app.toolbox["modelmaker_delft3dfm"].selected_bathymetry_datasets)
    if nrd>0:
        for dataset in app.toolbox["modelmaker_delft3dfm"].selected_bathymetry_datasets:
            selected_names.append(dataset["name"])
        app.gui.setvar(group, "selected_bathymetry_dataset_names", selected_names)
        index = app.gui.getvar(group, "selected_bathymetry_dataset_index")
        if index > nrd - 1:
            index = nrd - 1
        dataset = app.toolbox["modelmaker_delft3dfm"].selected_bathymetry_datasets[index]
...
def generate_bathymetry(*args):
    app.toolbox["modelmaker_delft3dfm"].generate_bathymetry()
