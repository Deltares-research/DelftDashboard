...
def delete_open_boundary_polygon(*args):
    ...
    # Delete from map
    gdf = app.map.layer["modelmaker_delft3dfm"].layer["open_boundary_polygon"].delete_feature(index)
...
def update():
    ...
    app.gui.setvar("modelmaker_delft3dfm", "open_boundary_polygon_names", names)
    index = app.gui.getvar("modelmaker_delft3dfm", "open_boundary_polygon_index")
    if index > nrp - 1:
        index = max(nrp - 1, 0)
    app.gui.setvar("modelmaker_delft3dfm", "open_boundary_polygon_index", index)
