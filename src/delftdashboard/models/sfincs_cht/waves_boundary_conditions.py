...
def select(*args):
    ...
    map.update()
    app.map.layer["sfincs_cht"].layer["boundary_points_snapwave"].activate()
    update_list()

def deselect(*args):
    if app.model["sfincs_cht"].wave_boundaries_changed:
...
def load(*args):
    ...
    app.model["sfincs_cht"].domain.snapwave.boundary_conditions.read_boundary_points()

    if read_bhs_file:
        # Read boundary conditions from bzs file
        app.model["sfincs_cht"].domain.snapwave.boundary_conditions.read_boundary_conditions_timeseries()
    else:
        # Set uniform conditions
        set_boundary_conditions()

    # Add boundary points to map
    gdf = app.model["sfincs_cht"].domain.snapwave.boundary_conditions.gdf
    app.map.layer["sfincs_cht"].layer["boundary_points_snapwave"].set_data(gdf, 0)
...
def point_clicked(x, y):
    ...
    app.model["sfincs_cht"].domain.snapwave.boundary_conditions.add_point(x, y)
    index = len(app.model["sfincs_cht"].domain.snapwave.boundary_conditions.gdf) - 1
    gdf = app.model["sfincs_cht"].domain.snapwave.boundary_conditions.gdf
    app.map.layer["sfincs_cht"].layer["boundary_points_snapwave"].set_data(gdf, index)
    app.gui.setvar("sfincs_cht", "active_boundary_point_snapwave", index)
    update_list()
    app.model["sfincs_cht"].wave_boundaries_changed = True

def select_boundary_point_from_list_snapwave(*args):
    index = app.gui.getvar("sfincs_cht", "active_boundary_point_snapwave")
    app.map.layer["sfincs_cht"].layer["boundary_points_snapwave"].select_by_index(index)
...
def delete_point_from_list_snapwave(*args):
    index = app.gui.getvar("sfincs_cht", "active_boundary_point_snapwave")
    app.model["sfincs_cht"].domain.snapwave.boundary_conditions.delete_point(index)
    gdf = app.model["sfincs_cht"].domain.snapwave.boundary_conditions.gdf
...
def update_list():
    ...
    nr_boundary_points = len(app.model["sfincs_cht"].domain.snapwave.boundary_conditions.gdf)
    boundary_point_names = []
    # Loop through boundary points
    for index, row in app.model["sfincs_cht"].domain.snapwave.boundary_conditions.gdf.iterrows():
        boundary_point_names.append(row["name"])
    app.gui.setvar("sfincs_cht", "boundary_point_names_snapwave", boundary_point_names)
    app.gui.setvar("sfincs_cht", "nr_boundary_points_snapwave", nr_boundary_points)
    app.gui.window.update()
...
def set_boundary_conditions_snapwave(*args):
    ...
    shape = app.gui.getvar("sfincs_cht", "boundary_conditions_timeseries_shape_snapwave")
    timestep = app.gui.getvar("sfincs_cht", "boundary_conditions_timeseries_time_step_snapwave")
    hm0 = app.gui.getvar("sfincs_cht", "boundary_conditions_timeseries_hm0_snapwave")
    tp  = app.gui.getvar("sfincs_cht", "boundary_conditions_timeseries_tp_snapwave")
    wd  = app.gui.getvar("sfincs_cht", "boundary_conditions_timeseries_wd_snapwave")
    ds  = app.gui.getvar("sfincs_cht", "boundary_conditions_timeseries_ds_snapwave")
    app.model["sfincs_cht"].domain.snapwave.boundary_conditions.set_timeseries_uniform(
                                                                      hm0,
                                                                      tp,
                                                                      wd,
                                                                      ds)
...
def create_boundary_points_snapwave(*args):
    ...
    if len(app.model["sfincs_cht"].domain.snapwave.boundary_conditions.gdf.index)>0:
    ...
    mask = app.model["sfincs_cht"].domain.grid.data["snapwave_mask"]
    if mask is None:
        ok = app.gui.window.dialog_info("Please first create a mask for this domain.",                                
                                    title=" ")
        return
    if not app.model["sfincs_cht"].domain.snapwave.mask.has_open_boundaries():
    ...
    bnd_dist = app.gui.getvar("sfincs_cht", "boundary_dx_snapwave")
    app.model["sfincs_cht"].domain.snapwave.boundary_conditions.get_boundary_points_from_mask(bnd_dist=bnd_dist)
    gdf = app.model["sfincs_cht"].domain.snapwave.boundary_conditions.gdf
    if len(gdf) == 0:
...
def get_bhs_file_name(*args):
    filename = app.model["sfincs_cht"].domain.input.variables.bhsfile
    if not filename:
        filename = "sfincs.bhs"
    rsp = app.gui.window.dialog_save_file("Select file ...",
                                        file_name=filename,
