...
def deselect(*args):
    if app.model["sfincs_hmt"].observation_points_changed:
        if app.model["sfincs_hmt"].domain.observation_points.nr_points == 0:
...
def update():
    ...
    app.gui.setvar("sfincs_hmt", "observation_point_name", name)
...
def edit_observation_point_name(*args):
    map.reset_cursor()
    index = app.gui.getvar("sfincs_hmt", "active_observation_point")
