...
def deselect(*args):
    if app.model["sfincs_hmt"].boundaries_changed:
...
def load(*args):
    ...
    if rsp[0]:
        app.model["sfincs_hmt"].domain.config.set("bndfile", rsp[2])
    else:
        # User pressed cancel
        return
    rsp = app.gui.window.dialog_open_file("Select file ...",
                                          ...
                                          allow_directory_change=False)
    if rsp[0]:
        app.model["sfincs_hmt"].domain.config.set("bzsfile", rsp[2])
        read_bzs_file = True
    else:
...
def save(*args, **kwargs):
    ...
    app.model["sfincs_hmt"].domain.config.set("bzsfile", bzsfile) # file name without path
    app.gui.setvar("sfincs_hmt", "bndfile", bndfile)
    app.gui.setvar("sfincs_hmt", "bzsfile", bzsfile)

    # # bca file
    # filename = app.model["sfincs_hmt"].domain.input.variables.bcafile
    # if not filename:
    #     filename = "sfincs.bca"
...
def generate_boundary_conditions_from_tide_model(*args):
    ...
    app.model["sfincs_hmt"].domain.config.set("bcafile", bcafile) # file name without path
    app.gui.setvar("sfincs_hmt", "bcafile", bcafile)
...
def set_boundary_conditions(*args):
    ...
    shape = app.gui.getvar("sfincs_hmt", "boundary_conditions_timeseries_shape")
    timestep = app.gui.getvar("sfincs_hmt", "boundary_conditions_timeseries_time_step")
    offset = app.gui.getvar("sfincs_hmt", "boundary_conditions_timeseries_offset")
    amplitude = app.gui.getvar("sfincs_hmt", "boundary_conditions_timeseries_amplitude")
    phase = app.gui.getvar("sfincs_hmt", "boundary_conditions_timeseries_phase")
    period = app.gui.getvar("sfincs_hmt", "boundary_conditions_timeseries_period")
...
def create_boundary_points(*args):
    ...
    mask = app.model["sfincs_hmt"].domain.quadtree_grid.data["mask"]
    if mask is None:
    ...
    app.map.layer["sfincs_hmt"].layer["boundary_points"].set_data(gdf, 0)
    # Set uniform conditions
    app.gui.setvar("sfincs_hmt", "boundary_conditions_timeseries_shape", "constant")
...
def get_bnd_file_name(*args):
    filename = app.model["sfincs_hmt"].domain.config.get("bndfile")
    if not filename:
        filename = "sfincs.bnd"
...
def get_bzs_file_name(*args):
    filename = app.model["sfincs_hmt"].domain.config.get("bzsfile")
    if not filename:
        filename = "sfincs.bzs"
    rsp = app.gui.window.dialog_save_file("Select file ...",
                                        file_name=filename,
...
def get_bca_file_name(*args):
    filename = app.model["sfincs_hmt"].domain.config.get("bcafile")
    if not filename:
        filename = "sfincs.bca"
    rsp = app.gui.window.dialog_save_file("Select file ...",
                                        file_name=filename,
