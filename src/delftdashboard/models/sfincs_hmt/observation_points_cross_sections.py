...
def select(*args):
    ...
    map.update()
    # Activate draw layer
    app.map.layer["sfincs_hmt"].layer["cross_sections"].layer["polylines"].activate()
    app.map.layer["sfincs_hmt"].layer["cross_sections"].layer["snapped"].activate()
    update()
...
def save(*args):
    map.reset_cursor()
    filename = app.model["sfincs_hmt"].domain.config.get("crsfile")
    if not filename:
        filename = "sfincs.crs"
    ...
    if rsp[0]:
        app.model["sfincs_hmt"].domain.config.set("crsfile", rsp[2]) # file name without path
        app.model["sfincs_hmt"].domain.cross_sections.write()
    app.model["sfincs_hmt"].cross_sections_changed = False
...
def delete_cross_section(*args):
    ...
    # Delete from map
    app.map.layer["sfincs_hmt"].layer["cross_sections"].layer["polylines"].delete_feature(index)
    # Delete from app
    app.model["sfincs_hmt"].domain.cross_sections.delete(index)
...
def select_cross_section_from_list(*args):
    map.reset_cursor()
    index = app.gui.getvar("sfincs_hmt", "active_cross_section")
    app.map.layer["sfincs_hmt"].layer["cross_sections"].layer["polylines"].activate_feature(index)
    update()
...
def cross_section_created(gdf, index, id):
    ...
    if not okay:
        # Cancel was clicked
        return    
    if name in app.gui.getvar("sfincs_hmt", "cross_section_names"):
        ...
        while name in app.gui.getvar("sfincs_hmt", "cross_section_names"):
            name, okay = app.gui.window.dialog_string("Edit name for new cross section")
            if not okay:
                # Cancel was clicked
                return
...
def cross_section_selected(index):
    app.gui.setvar("sfincs_hmt", "active_cross_section", index)
    update()
...
def update_grid_snapper():
    snap_gdf = app.model["sfincs_hmt"].domain.cross_sections.snap_to_grid()
    if len(snap_gdf) > 0:
...
def update():
    ...
    if nrt > 0:
        ...
        app.gui.setvar("sfincs_hmt", "cross_section_name", "")
