...
def load(*args):
    ...
    if rsp[0]:
        ...
        gdf = app.model["sfincs_hmt"].domain.thin_dams.gdf
...
def thin_dam_created(gdf, index, id):
    app.model["sfincs_hmt"].domain.thin_dams.set(gdf, merge=False)
    nrt = len(gdf)
...
def update_grid_snapper():
    ...
    if len(snap_gdf) > 0:
        ...
        app.map.layer["sfincs_hmt"].layer["thin_dams"].layer["snapped"].clear()

def update():
    nrt = len(app.model["sfincs_hmt"].domain.thin_dams.gdf)
