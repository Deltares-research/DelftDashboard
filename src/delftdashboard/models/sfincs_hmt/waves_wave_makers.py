...
def delete_from_list(*args):
    ...
    app.model["sfincs_hmt"].domain.wave_makers.delete_polyline(index)
...
def wave_maker_created(gdf, index, id):
    app.model["sfincs_hmt"].domain.wave_makers.gdf = gdf
...
def load(*args):
    ...
    if rsp[0]:
        app.model["sfincs_hmt"].domain.input.variables.wvmfile = rsp[2] # file name without path
...
def save(*args):
    filename = app.model["sfincs_hmt"].domain.input.variables.wvmfile
    if not filename:
        filename = "sfincs.wvm"
