...
def load(*args):
    ...
    rsp = app.gui.window.dialog_open_file(
        "Select file with observation points ...", filter="*.obs *.shp *.geojson"
    )
    ...
    if rsp[0]:
        ...
        if len(gdf.index) == 0:
            app.gui.window.dialog_info(
                "No valid observation points found within model domain!"
            )
            return
        ...
        ok = app.gui.window.dialog_yes_no(
            "Do you want to merge the observation points with the existing ones?"
        )
