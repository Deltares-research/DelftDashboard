...
def load(*args):
    ...
    if rsp[0]:
        app.model["sfincs_hmt"].domain.input.variables.obsfile = rsp[2] # file name without path
        app.model["sfincs_hmt"].domain.observation_points.read()
        gdf = app.model["sfincs_hmt"].domain.observation_points.gdf
...
def save(*args):
    map.reset_cursor()
    file_name = app.model["sfincs_hmt"].domain.input.variables.obsfile
    if not file_name:
        file_name = "sfincs_hmt.obs"
...
def update():
    gdf = app.active_model.domain.geoms["obs"].gdf
    names = []
...
def point_clicked(x, y):
    ...
    index = len(app.model["sfincs_hmt"].domain.observation_points.gdf) - 1
    gdf = app.model["sfincs_hmt"].domain.observation_points.gdf
...
def delete_point_from_list(*args):
    map.reset_cursor()
    index = app.gui.getvar("sfincs_hmt", "active_observation_point")
    app.model["sfincs_hmt"].domain.observation_points.delete_point(index)
    gdf = app.model["sfincs_hmt"].domain.observation_points.gdf
