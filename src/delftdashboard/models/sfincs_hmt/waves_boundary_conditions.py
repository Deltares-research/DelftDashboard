...
def load(*args):
    ...
    if rsp[0]:
        app.model["sfincs_hmt"].domain.input.variables.snapwave_bndfile = rsp[2] # file name without path
    else:
        # User pressed cancel
        return
    rsp = app.gui.window.dialog_open_file("Select file ...",
                                          ...
                                          allow_directory_change=False)
    if rsp[0]:
        app.model["sfincs_hmt"].domain.input.variables.snapwave_bhsfile = rsp[2] # file name without path
        read_bhs_file = True
    else:
    ...
    app.model["sfincs_hmt"].domain.snapwave.boundary_conditions.read_boundary_points()

    if read_bhs_file:
        # Read boundary conditions from bzs file
        app.model["sfincs_hmt"].domain.snapwave.boundary_conditions.read_boundary_conditions_timeseries()
    else:
        # Set uniform conditions
        set_boundary_conditions()

    # Add boundary points to map
    gdf = app.model["sfincs_hmt"].domain.snapwave.boundary_conditions.gdf
...
def save(*args, **kwargs):
    ...
    app.model["sfincs_hmt"].domain.input.variables.snapwave_bndfile = bndfile # file name without path
    app.gui.setvar("sfincs_hmt", "snapwave_bndfile", bndfile)

    # Write bnd and bhs files
    app.model["sfincs_hmt"].domain.snapwave.boundary_conditions.write_boundary_points()
    app.model["sfincs_hmt"].domain.snapwave.boundary_conditions.write_boundary_conditions_timeseries()
    app.model["sfincs_hmt"].wave_boundaries_changed = False
...
def point_clicked(x, y):
    ...
    app.model["sfincs_hmt"].domain.snapwave.boundary_conditions.add_point(x, y)
    index = len(app.model["sfincs_hmt"].domain.snapwave.boundary_conditions.gdf) - 1
    gdf = app.model["sfincs_hmt"].domain.snapwave.boundary_conditions.gdf
...
def delete_point_from_list_snapwave(*args):
    index = app.gui.getvar("sfincs_hmt", "active_boundary_point_snapwave")
    app.model["sfincs_hmt"].domain.snapwave.boundary_conditions.delete_point(index)
    gdf = app.model["sfincs_hmt"].domain.snapwave.boundary_conditions.gdf
...
def update_list():
    ...
    nr_boundary_points = len(app.model["sfincs_hmt"].domain.snapwave.boundary_conditions.gdf)
    boundary_point_names = []
    # Loop through boundary points
    for index, row in app.model["sfincs_hmt"].domain.snapwave.boundary_conditions.gdf.iterrows():
...
def set_boundary_conditions_snapwave(*args):
    ...
    # app.model["sfincs_hmt"].domain.input.variables.bhsfile = bhsfile # file name without path
    # app.gui.setvar("sfincs_hmt", "bhsfile", bhsfile)

    dlg = app.gui.window.dialog_wait("Generating boundary conditions ...")

    shape = app.gui.getvar("sfincs_hmt", "boundary_conditions_timeseries_shape_snapwave")
    timestep = app.gui.getvar("sfincs_hmt", "boundary_conditions_timeseries_time_step_snapwave")
    ...
    app.model["sfincs_hmt"].domain.snapwave.boundary_conditions.set_timeseries_uniform(
                                                                      hm0,
                                                                      tp,
                                                                      wd,
                                                                      ds)
...
def create_boundary_points_snapwave(*args):
    ...
    if len(app.model["sfincs_hmt"].domain.snapwave.boundary_conditions.gdf.index)>0:
    ...
    app.model["sfincs_hmt"].domain.snapwave.boundary_conditions.get_boundary_points_from_mask(bnd_dist=bnd_dist)
    gdf = app.model["sfincs_hmt"].domain.snapwave.boundary_conditions.gdf
    if len(gdf) == 0:
