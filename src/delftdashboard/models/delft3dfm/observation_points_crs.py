...
def delete_obs_line(*args):
    ...
    if len(gdf) == 0:
        return
    index = app.gui.getvar("delft3dfm", "observation_line_index")
    # Delete from app
...
def update():
    ...
    # if app.gui.getvar("delft3dfm", "observation_line_index" ) >  len(gdf) - 1:
    #     app.gui.setvar("delft3dfm", "observation_line_index", max( len(gdf) - 1, 0) )
